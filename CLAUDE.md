...
- Trial end (not converted) → `expired` → org read-only. `SubscriptionExpiryJob` (nightly 00:05 UTC, `app.jobs.trial-expiry.enabled` / cron config, enabled in all profiles — `test` disables it) flips lapsed `TRIALING` rows `EXPIRED`; paid rows are never touched here (Razorpay webhooks own those).
